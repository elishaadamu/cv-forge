      ...
      <footer className="pt-32 pb-16 px-4 bg-brand-primary text-white relative z-10 border-t border-white/5">
        <div className="max-w-7xl mx-auto">
          <div className="grid grid-cols-1 lg:grid-cols-4 gap-16 mb-20">
             <div className="lg:col-span-2 space-y-8">
               ...
               <div className="flex space-x-5">
                  {[
                    ...
                    { Icon: Heart, name: "Donate" },
                    { Icon: Star, name: "Star us" }
                  ].map((item, i) => (
                    <Link key={i} href="#" aria-label={item.name} className="w-12 h-12 bg-white/5 rounded-2xl flex items-center justify-center text-white/50 hover:text-brand-action hover:scale-110 transition-all duration-300">
             ...
             <div className="space-y-8">
                <h3 className="text-sm font-black uppercase tracking-[0.2em] text-white/50">Company</h3>
                <div className="flex flex-col space-y-5">
                   {['Our Vision', 'Privacy Policy', 'Terms of Service', 'Support'].map(item => (
                     <Link key={item} href="#" className="font-bold text-lg text-white/90 hover:text-brand-action transition-colors">{item}</Link>
                   ))}
                </div>
             </div>
          </div>
